	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 15}) then {
			_text = switch (true) do {
				case ((goggles _x) in D41_goggles_Array): {format["<t color='#000000'>Maskierte Person</t>"];};
				case ((headgear _x) in D41_headgear_Array): {format["<t color='#000000'>Maskierte Person</t>"];};
				case ((uniform _x) in D41_uniform_Array): {format["<t color='#000000'>Maskierte Person</t>"];};
				case ((hmd _x) in D41_hmd_Array): {format["<t color='#000000'>Maskierte Person</t>"];};
				case ((!isNil{_x getVariable "D41_FakeID"}) && ("D41_FalscherAusweis" in magazines _x)): {format["<t color='#CCFFFF'>%1</t>",(_x getVariable "D41_FakeID")];};
