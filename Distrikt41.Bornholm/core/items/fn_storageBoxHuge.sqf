...
if(isNil "_house")exitWith{systemchat "FEHLER! BEIM ADMIN MELDEN! HOUSING SYSTEM StorageBoxHuge! - IsNil"};
if(count(_house) == 0)exitWith{systemchat "FEHLER! BEIM ADMIN MELDEN! HOUSING SYSTEM StorageBoxHuge!"};
_house = _house select 0;
if(!(_house in life_vehicles)) exitWith {hint localize "STR_D41_no_House"};
if(!(_house isKindOf "Land_i_Shed_Ind_F")) exitWith {hint localize "STR_D41_no_Warehouse"};
...
if(count _containers >= (_houseCfg select 1)) exitWith {hint localize "STR_D41_House_no_Box_Space"};
...
if(!("D41_LagerkisteLagerhaus" in magazines player))exitWith{hint "Wat? Wie kommste hier hin? Am bescheißen oder was??"};

	player removeMagazine "D41_LagerkisteLagerhaus";
	_Hauscontainer = "D41_LagerhausContainer_Blue" createVehicle [0,0,0];
