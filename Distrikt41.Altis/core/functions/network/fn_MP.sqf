/*
	...
	ARRAY - sent packet
*/
	//Vorbereitungen für remoteExec || Muß ausgiebig getestet werden, mach ich 1 Tag vor V1.2 nimmer... ... ...
	...
	if((typeName _target) == "BOOL")then
	{
		switch(true)do
		{
			case(!_target): {_target = 2};
			case(_target): {_target = 0};
		};
	};
	
	diag_log format ["life_fnc_mp: _this = %1", _this];
	diag_log format ["life_fnc_mp: _params = %1", _params];
	diag_log format ["life_fnc_mp: _functionName = %1", _functionName];
	diag_log format ["life_fnc_mp: _target = %1", _target];
	diag_log format ["life_fnc_mp: _isPersistent = %1", _isPersistent];
	diag_log format ["life_fnc_mp: _isCall = %1", _isCall];
