...
*/
private["_vehicles","_control","_ImpoundMenu"];
disableSerialization;
_vehicles = [_this,0,[],[[]]] call BIS_fnc_param;
_ImpoundMenu = _this select 1;
if(_ImpoundMenu)then{D41_ImpoundMenu = true}else{D41_ImpoundMenu = false};

ctrlShow[2803,false];
ctrlShow[2830,false];
waitUntil {!isNull (findDisplay 2800)};

if(count (_vehicles select 0) == 0) exitWith
{
