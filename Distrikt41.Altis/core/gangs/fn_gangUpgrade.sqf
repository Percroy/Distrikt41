...
_maxMembers = grpPlayer getVariable ["gang_maxMembers",2];
if(_maxMembers == 8)exitWith{hint localize "STR_D41_Gang_MaxMember";};
...
if(_action) then {
	if(D41_atmGeld < _upgradePrice) exitWith {
		hint parseText format[
			...
			[D41_atmGeld] call life_fnc_numberText,
			[_upgradePrice - D41_atmGeld] call life_fnc_numberText
		];
	};
	__SUB__(D41_atmGeld,_upgradePrice);
