...
private["_rndm","_amount","_type","_remove","_Msg","_Display"];
	...
	if(life_D41_Karma >= -3000) exitWith {systemchat format [localize "STR_D41_Karma_left",(life_D41_Karma + 2999)];};
	
	D41_Counter = D41_Counter + 1;
	if(D41_Counter >= 100) exitWith {systemchat localize "STR_D41_Lucky_Skill"};
	
	_Msg = {systemchat format [localize "STR_D41_Skill_lost", _amount, _Display]};
	...
	if (_rndm == 2) then	{if(life_skillTon < 199) exitWith {call life_fnc_D41_KillMalus};		_type = "D41_TonRoh";		_Display = "Tonerde";	_amount = "100";	life_skillTon = life_skillTon - 100;				call _remove;};
