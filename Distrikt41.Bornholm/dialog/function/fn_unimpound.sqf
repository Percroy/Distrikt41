...
if(D41_ImpoundMenu)then
{
	...
	if(_CopADACNearby)then{_price = _price * 5}else{_price = _price * 15};
};

if(D41_atmGeld < _price) exitWith {hint format[(localize "STR_Garage_CashError"),[_price] call life_fnc_numberText];};
