...
_RadioRequest = format["radio_request_%1%2",getPlayerUID player,playerside];
_RadioResponse = format["radio_response_%1%2",getPlayerUID player,playerside];
...
{
	if(_x != _RadioResponse)then
	{
		if(_x != _RadioRequest)then
		{
							...
							if(!(_x in _SOCK_Functions)) then
							{
								if(!(_x in _DB_Functions)) then
								{
									...
									if(EQUAL(_find,-1)) then
									{
										...
										systemchat format ["UNBEKANNTE FUNKTION GEFUNDEN! LOGFILES AN DSCHA! ||  %1  ||", _x];
								...
								if(!(_x in _DB_Functions)) then
								{
									if(!(_x in _BIS_UI_Functions)) then
									{
										...
										if(EQUAL(_find,-1)) then
										{
											...
											systemchat format ["UNBEKANNTE UI-FUNKTION GEFUNDEN! LOGFILES AN DSCHA! ||  %1  ||", _x];
											//failMission "SpyGlass";
										};
									};
								};
