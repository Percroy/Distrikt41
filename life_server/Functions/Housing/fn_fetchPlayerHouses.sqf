...
diag_log format ["FETCHPLAYER HOUSES: _GangQueryResult   %1",_GangQueryResult];
_query = format["SELECT pid, pos, containers FROM houses WHERE (pid='%1' OR gid='%2') AND owned='1'",_this, _GangQueryResult select 0];
waitUntil{!DB_Async_Active};
diag_log format ["FETCHPLAYER HOUSES: _query   %1",_query];
_houses = [_query,2,true] call DB_fnc_asyncCall;

{
		_HouseCheck2 = true;
		_pos = call compile format["%1",_x select 1];
	    _HouseCheck = nearestObjects [_pos,["D41_Box_IND_Grenades_F","D41_supplyCrate_F","D41_LagerhausKiste_F"], 5];
		...
		if(typeName _containerData == "STRING") then {_containerData = call compile format["%1", _containerData];};
		{
				...
				if(_pos isEqualTo [0,0,0]) exitWith {diag_log format ["FETCH PLAYER HOUSES: PlayerID - Fetching skipped %1", _this]};
