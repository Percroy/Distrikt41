...
if(player distance _curTarget > _distance) exitWith {hint localize "STR_D41_SlenderMan"; player addMagazine "D41_Dietrich";}; //Too far
...
if(!_isVehicle) then {
	...
	if(_dice < 30) then {
		...
		if(playerside == civilian)then{ [[getPlayerUID player,profileName,"214"],"life_fnc_wantedAdd",false,false] call life_fnc_MP; };
		player addMagazine "D41_Dietrich";
	} else {
		if(playerside == civilian)then{ [[getPlayerUID player,profileName,"215"],"life_fnc_wantedAdd",false,false] call life_fnc_MP; };
