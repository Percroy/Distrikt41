...
while {true} do {
	...
	if(!isNull _display && {count (allControls _display) > 103}) then {
		_count = count allControls _display;
		[[profileName,getPlayerUID player,format["MenuBasedHack_RscDisplayInventory_Controls_%1",_count]],"SPY_fnc_cookieJar",false,false] call life_fnc_MP;
		[[profileName,format["Menu Hack: RscDisplayInventory number of controls do not match (Count %1)",_count]],"SPY_fnc_notifyAdmins",true,false] spawn life_fnc_MP;
		closeDialog 0;
